...
public class WebpNativeLibHelper {
  ...
  public static String getLibName() {
    String baseName = Tools.Companion.is64Bit() ? "webp_jni64" : "webp_jni";
    String fileName = System.mapLibraryName(baseName);
    if (Tools.Companion.isMac()) {
  ...
  public static File getLibLocation() {
    String libPath = "/tools/mctools/libwebp/" + getPlatformName();
    String home = System.getProperty("user.dir");
    return new File(new File(home).getParent(),libPath);
  }


  private static String getPlatformName() {
    if (Tools.Companion.isWindows()) return "win";
    else if (Tools.Companion.isMac()) return "mac";
    else if (Tools.Companion.isLinux()) return "linux";
    else return "";
  }
}
